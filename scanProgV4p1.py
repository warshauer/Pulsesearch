    ...
    def _parseDelaysHeater(self):
        ...
        for item in tl:
            ...
            for heatCom in thisHeatCommand:
                ...
                if len(yessir) < 4:
                    heaterCommand.append([float(yessir[0]), int(yessir[1]), float(yessir[2])*60, None])
                else:
                    heaterCommand.append([float(yessir[0]), int(yessir[1]), float(yessir[2])*60, float(yessir[3])])
            delays = []
            numScans = []
                        ...
                        for jj in range(len(srPositions)):
                            if jj == 0:
                                for k in range(numScans[j]):
                                    ...
                                    if type(samppos) != float:
                                        xStart = srPositions[jj][0]
                                    else:
                                        xStart = samppos
                                    xKerby = {'stage_key':xKey, 'start':xStart, 'moving':False, 'stepsize':0, 'subdir':True, 'subdirname':'{:0.1f}K_'.format(scanningTemp)+dirnames[jj]}
