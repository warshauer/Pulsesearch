...
class DLscanWindow(QtWidgets.QWidget):
    def __init__(self, parent, whoami = 'DLscan'):
        ...
        self.ui = uic.loadUi('scans.ui',self)
    ...
    def _moveToStartPositions(self, delay = 0.0):
        ...
        self.esp.move_to_position(self.sWidgets['THzindex'].value(), self.THzStart)
        while self.esp.moving():
            time.sleep(.1)
        self.esp.move_to_position(self.sWidgets['gateindex'].value(), self.sWidgets['gatesetposition'].value() - self.pn*self.link*self.sWidgets['prescan'].value() - delay*.15)
        while self.esp.moving():
            time.sleep(.1)
        time.sleep(4.0)
    ...
    def _moveStep(self):
        step = self.pn*self.sWidgets['stepsize'].value()*.001
        self.esp.move_step(self.sWidgets['THzindex'].value(), step)
        if self.link != 0:
            self.esp.move_step(self.sWidgets['gateindex'].value(), step*self.link)
        self.motionAllowed = False
        self.stageJustMoved = True

    def _update_stage_positions(self):
        self.stageValues[1]['pos'], self.stageValues[2]['pos'], self.stageValues[3]['pos'] = self.esp.positions()
    ...
    def appendData(self):
        ...
        self.x.append(x)
        self.moveOn, self.delay, self.di, self.si, self.ri = self.dataMuncher.feedData(x, y1, y2)
    ...
    def _update_scan_numbers(self):
        self.sWidgets['round'].setText(str(self.ri))
        self.sWidgets['delay'].setText(format(self.delay, '.2f'))
        self.sWidgets['scan'].setText(str(self.si))
